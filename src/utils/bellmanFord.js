    ...
    for (let x = 0; x < map.rows; x++) {
        for (let y = 0; y < map.cols; y++) {
            if (x == startX && y == startY)
                edges[map.width * x + y] = new WeightedNode(0, x, y, null, null);
            else if (map.isBlockFree(x, y))
                edges[map.width * x + y] = new WeightedNode(Infinity, x, y, null, null);
        }
    }
        ...
        edges.forEach(edge => {
            getAdjBF(edge.x, edge.y, map).forEach(adj => {
                ...
                if (edges[map.width * adj[0] + adj[1]].distance > custo) {
                    edges[map.width * adj[0] + adj[1]] = new WeightedNode(custo, adj[0], adj[1], edge.x, edge.y);
                    changes = true;
                }
            });
        });
